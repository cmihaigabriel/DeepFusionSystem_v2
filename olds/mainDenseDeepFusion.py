...
from CrossSpaceFusionLayer import CSF3var

def runFusion(no_layers = -1, no_neurons = -1, database = -1, bn_active = 0, kfolds = -1, repeatfolds = -1, gpudevice = "-1", attnLayer = 0, convLayer = 0, csfLayer = 0):
    ...
    class Settings(keras.callbacks.Callback):
        ...
        def loss_mapper(self, losses, y_pred, thisloss):
            if database in [0,1,2]:
                ...
                y_class = []
                thresh = serializer.calc2DThresh(y_pred)
                for i in range(len(y_pred)):
                    ...
                    for j in range(len(yp)):
                        if yp[j] > thresh[j]:
                            ...
                            ytmp.append(0)
                    y_class.append(ytmp)
                curmetric = serializer.calcF1(y_class, y_valid_cv)
                cursecmetric = 0.0
                ...
                y_class = []
                thresh = serializer.calc1DThresh(y_pred)
                for i in range(len(y_pred)):
                    if (y_pred[i] < thresh):
                        y_class.append(0)
                    else:
    ...
    def get_model_seq():
        ...
        if convLayer > 0:
            ...
            model.add(Flatten())
        if csfLayer > 0:
            model.add(CSF3var(force_unitcenter=True, input_shape=(x_train_cv.shape[1], x_train_cv.shape[2], x_train_cv.shape[3])))
            model.add(AveragePooling1D(pool_size=(3)))
            model.add(AveragePooling1D(pool_size=(3), data_format='channels_first'))
            model.add(Flatten())

        if database == 3:
            model.add(Flatten(input_shape=(x_train_cv.shape[1], x_train_cv.shape[2],)))
            model.add(Dense(no_neurons, activation='relu', input_shape=(int(x_train_cv.shape[1] * x_train_cv.shape[2]),)))
        else:
    ...
    def get_model_attntype():
        if database == 3:
            input_layer = Input(shape=(input.shape[1],x_train_cv.shape[2],))
            flatten_layer = Flatten(input_shape=(x_train_cv.shape[1], x_train_cv.shape[2],))(input_layer)
        ...
        if database == 3:
            attention_probs = Dense(num_classes, activation='softmax', name='attention_probs')(flatten_layer)
        else :
    ...
    for j, (train_idx, val_idx) in enumerate(folds):
        ...
        if csfLayer == 1:
            (x_train_cv, x_valid_cv) = matrixCreator.getMatrix_CSF32_4N4S4C(x_train_cv, x_valid_cv,rmetric)
        if csfLayer == 2:
            (x_train_cv, x_valid_cv) = matrixCreator.getMatrix_CSF32_8N8S8C(x_train_cv, x_valid_cv,rmetric)
...
def SetupFileHandler(ser, type):
    ...
    #####MEDCapt2019
    if (type == 3):
        ser.setup('../DataSources/ImageCLEFMedical/MEDCapt2019/SortedRuns',
                  '../FusionCache/FusionResults3/',
                  'conMEDCapt2019',
                  '../DataSources/ImageCLEFMedical/MEDCapt2019/GT/ConceptDetection_GTmask.txt',
